...
type ArtifactQuery struct {
	...
	withParent     *ArtifactQuery
	withChildren   *ArtifactQuery
	withFKs        bool
...
func (aq *ArtifactQuery) QueryRepository() *RepositoryQuery {
...
func (aq *ArtifactQuery) QueryParent() *ArtifactQuery {
	query := (&ArtifactClient{config: aq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (aq *ArtifactQuery) QueryChildren() *ArtifactQuery {
	query := (&ArtifactClient{config: aq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (aq *ArtifactQuery) FirstID(ctx context.Context) (id xid.ID, err error) {
	var ids []xid.ID
...
func (aq *ArtifactQuery) FirstIDX(ctx context.Context) xid.ID {
...
func (aq *ArtifactQuery) OnlyID(ctx context.Context) (id xid.ID, err error) {
	var ids []xid.ID
...
func (aq *ArtifactQuery) OnlyIDX(ctx context.Context) xid.ID {
...
func (aq *ArtifactQuery) IDs(ctx context.Context) (ids []xid.ID, err error) {
...
func (aq *ArtifactQuery) IDsX(ctx context.Context) []xid.ID {
	ids, err := aq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (aq *ArtifactQuery) WithRepository(opts ...func(*RepositoryQuery)) *ArtifactQuery {
	query := (&RepositoryClient{config: aq.config}).Query()
...
func (aq *ArtifactQuery) WithParent(opts ...func(*ArtifactQuery)) *ArtifactQuery {
	query := (&ArtifactClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (aq *ArtifactQuery) WithChildren(opts ...func(*ArtifactQuery)) *ArtifactQuery {
	query := (&ArtifactClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (aq *ArtifactQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Artifact, error) {
	...
	if aq.withRepository != nil || aq.withParent != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := aq.withRepository; query != nil {
		if err := aq.loadRepository(ctx, query, nodes, nil,
			func(n *Artifact, e *Repository) { n.Edges.Repository = e }); err != nil {
			return nil, err
		}
	}
	if query := aq.withParent; query != nil {
		if err := aq.loadParent(ctx, query, nodes, nil,
			func(n *Artifact, e *Artifact) { n.Edges.Parent = e }); err != nil {
			return nil, err
		}
	}
	if query := aq.withChildren; query != nil {
		if err := aq.loadChildren(ctx, query, nodes,
			func(n *Artifact) { n.Edges.Children = []*Artifact{} },
			func(n *Artifact, e *Artifact) { n.Edges.Children = append(n.Edges.Children, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *ArtifactQuery) loadRepository(ctx context.Context, query *RepositoryQuery, nodes []*Artifact, init func(*Artifact), assign func(*Artifact, *Repository)) error {
	ids := make([]xid.ID, 0, len(nodes))
	nodeids := make(map[xid.ID][]*Artifact)
	for i := range nodes {
		if nodes[i].repository_artifacts == nil {
			continue
		}
		fk := *nodes[i].repository_artifacts
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "repository_artifacts" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (aq *ArtifactQuery) loadParent(ctx context.Context, query *ArtifactQuery, nodes []*Artifact, init func(*Artifact), assign func(*Artifact, *Artifact)) error {
	ids := make([]xid.ID, 0, len(nodes))
	nodeids := make(map[xid.ID][]*Artifact)
	for i := range nodes {
		if nodes[i].artifact_children == nil {
			continue
		}
		fk := *nodes[i].artifact_children
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "artifact_children" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (aq *ArtifactQuery) loadChildren(ctx context.Context, query *ArtifactQuery, nodes []*Artifact, init func(*Artifact), assign func(*Artifact, *Artifact)) error {
	...
	query.Where(predicate.Artifact(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(artifact.ChildrenColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.artifact_children
		if fk == nil {
			return fmt.Errorf(`foreign-key "artifact_children" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "artifact_children" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (aq *ArtifactQuery) ForUpdate(opts ...sql.LockOption) *ArtifactQuery {
...
func (aq *ArtifactQuery) ForShare(opts ...sql.LockOption) *ArtifactQuery {
	if aq.driver.Dialect() == dialect.Postgres {
		aq.Unique(false)
	}
	aq.modifiers = append(aq.modifiers, func(s *sql.Selector) {
...
func (aq *ArtifactQuery) Modify(modifiers ...func(s *sql.Selector)) *ArtifactSelect {
...
// Modify adds a query modifier for attaching custom logic to queries.
func (as *ArtifactSelect) Modify(modifiers ...func(s *sql.Selector)) *ArtifactSelect {
